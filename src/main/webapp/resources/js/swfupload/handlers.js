...
function fileQueued(file){
	addReadyFileInfo(file.size,file.id,file.name,"等待上传");
	//swfu.startUpload();
}
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesQueued > 0) {
			...
			//addReadyFileInfo(file.id,file.name,"队列中");
			this.startUpload();
		}
	} catch (ex) {
		this.debug(ex);
...
function addFileInfo(fileId,message){
	...
	$(row).find(".f_status").text(message);
}
function addReadyFileInfo(filesize,fileid,fileName,message,status){
	$("#thumbnails").css("display","block");
	$("#infoList ul").append('<li id="'+fileid+
			'"><span class="f_name">'+fileName+
			'</span><span class="f_size">'+filesize+
			'</span><span class="f_status">'+message+
			'</span><span class="f_del">删除</span></li>');
}
